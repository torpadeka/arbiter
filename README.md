...
Nine business tools, one canonical knowledge graph in [HydraDB](https://github.com/hydra-db/hydradb), and answers that arrive with the traversal path that produced them — or don't arrive at all.
...
A company's knowledge is smeared across Slack, Gmail, Linear, Jira, Confluence, Drive, GitHub, HubSpot and Fireflies. The corpus is hostile in three specific ways, and a vector index handles none of them:
...
Arbiter answers a different question from "what does the corpus say". It answers **what is currently true, who established it, when — or nothing, if the corpus doesn't say.**
...
Abstention precision 100%, recall 100% — zero over-abstentions, zero hallucination-risk answers. Latency p50 4 ms, p95 5 ms.

24 of the 28 questions are answerable from the deterministic pipeline alone; 4 need tier B and are skipped automatically when the graph was built without it, rather than counted as failures.

**The baseline is scored generously on purpose.** It is TF-IDF retrieval over the same corpus, and it only has to *retrieve a document containing the answer* in its top 5 — no extraction, no reasoning, no penalty for picking the wrong candidate among several. It still scores 0% on contradictions (it cannot tell stale from current) and 0% on unanswerable questions (it never declines). Its 100% on alias questions is real and worth stating: when a question and its answer happen to share a document, term matching finds it.

## Results on a real dataset: Salesforce HERB

The seed suite is ours, so it can be tuned to pass. [HERB](https://huggingface.co/datasets/Salesforce/HERB) cannot be — 39,190 artifacts across 30 products, with its own questions and its own ground truth.
...
- **HERB has no thread structure.** `ThreadReplies` is empty in all 1,084 messages; a conversation is a run of top-level messages sharing a channel and date. Grouping them is what makes "who reviewed this document" reachable at all.
- **Documents are cited by URL**, not by field — `<https://…/docs/onforcex_market_research_report|Market Research Report>` — so citations become `REFERENCES` edges from every message in the citing conversation.
...
python cli.py ask "who owns Atlas Migration?" --as-of 2026-03-15         # ...and who owned it in March
...
The last one matters more than it looks. Wei sits one hop from Sam, whose reporting line *is* recorded — the kind of adjacency that makes a retrieval system answer confidently about the wrong person.
...
| Batched edges take one type and no properties | All provenance lives on the `Claim` node; status is encoded in the relationship type (`SCHEDULED_FOR` vs `SCHEDULED_FOR_SUPERSEDED`), keeping traversal filterable |
| Node ids must be non-negative integers | Keys are hashed (`blake2b(key)[:8] >> 1`), with the readable key kept as a property |

The full verified capability matrix — including the nine query forms the engine rejects and the workaround for each — is in [`docs/hydradb-capabilities.md`](docs/hydradb-capabilities.md), reproducible with `python scripts/spike_hydra.py`.

> **Deployment note:** `CLOUD_PROVIDER=local` accepts a few writes and then fails every mutation, because `LocalFileSystem` does not implement the conditional puts SlateDB needs for the writer lease. `scripts/hydradb_up.ps1` therefore runs MinIO as the object store.
...
- **Alignment (`resolve/align.py`).** Prose does not speak in canonical keys, and this is where most of the ontology work actually happens. Four passes, each conservative and each schema-driven:
  - *surface* — strip determiners, then align to a known entity only on strong evidence (`the Atlas migration` → `Atlas Migration`). A structured field outranks the extractor's type guess, but only on exact containment: `Atlas launch` retypes to the Linear ticket rather than minting a project.
  - *orientation* — flip claims whose types violate domain/range but satisfy it reversed.
  - *same relation, different word* — `remap` in the schema: "Atlas is assigned to Sam" said of a **project** means ownership, said of a **ticket** means assignment.
  - *functional form* — rewrite a multi-valued predicate to its functional inverse (`OWNS` → `OWNED_BY`) so competing claims land in one arbitrable set.
...
- **Entity resolution.** Normalize → block → score → **veto** → union-find. Three vetoes are absolute: conflicting emails, conflicting surnames, and *addressed-by-name* (an author who writes "Thanks Priya" is not Priya). Every merge stores its evidence, which the CLI displays.
- **Arbitration.** A published, tunable formula — `0.35·authority + 0.30·recency + 0.15·specificity + 0.15·corroboration − 0.05·hedging` — with a source-authority table (Jira 1.00 … Slack 0.50). Losing claims are **never deleted**; they are marked superseded and linked with `:SUPERSEDES`. Conflicts are only detected for *functional* predicates: two tickets assigned to one person is not a contradiction.
...
| 2 · Coverage | Is the asked-about predicate present? | *Found atlas migration, but nothing recorded about its budget. Recorded: customer relationship, discussion, scheduled date, work.* |
...
```
ontology/schema.yaml     node types, predicate vocabulary (cardinality, temporality, inverse, remap), 9 source field maps
...
docs/                    verified HydraDB capability matrix and measured throughput
...
Full-corpus scale is not a projection: parse, resolve, arbitrate and assemble take **under 20 seconds** for 39k documents, and the graph writes in about ten minutes at ~1,000 elements/sec with no query timeouts. Entity resolution grows *sub*-linearly as the employee pool saturates, because blocking keeps comparisons inside buckets.

## Scope and honesty

- **Two corpora, deliberately.** The seed corpus is synthetic — 28 documents across 9 tools in native record shapes, with planted alias clusters, contradictions, near-duplicates, gaps, and an over-merge trap (two different people named Priya in one thread). It exists so entity resolution, arbitration and abstention can be measured against ground truth *we control*; HERB's questions cannot test a trap nobody planted. HERB is the check that none of it was overfitted to our own data. Both are reported above, including where HERB goes badly.
- **HERB results are from one product of thirty.** Loading more is a flag (`--products N`), and the scale test shows the full corpus fits, but every number above is from what was actually run, not extrapolated.
- **Tiered ingestion is a deliberate cost decision**, documented rather than hidden: deterministic parsing runs over every document; LLM extraction runs over free text only, capped by `TIER_B_DOC_LIMIT`.
- Model defaults are the most capable model. Extraction and alias adjudication are high-volume mechanical calls where `claude-haiku-4-5` is a reasonable trade — that switch is left to the operator in `.env`, not made silently.
